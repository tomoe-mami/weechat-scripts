...
local g = {
   script = {
      ...
      license = "WTFPL",
      version = "0.1",
      description = "Script for inserting UTF-8 character using codepoint or mnemonic"
   },
   config = {},
   defaults = {
      show_mnemonics_table = {
         value = "0",
         type = "boolean",
         description = "Show table of characters when using mnemonic mode"
      },
      mode_color = {
         value = "green",
         description = "Color for mode indicator"
      },
      char_color = {
         value = "lightblue",
         description = "Color for character in mnemonics table"
      },
      code_color = {
         value = "default",
         description = "Color for mnemonic code in mnemonics table"
      }
   }
}
...
function init_config()
   for name, info in pairs(g.defaults) do
      if w.config_is_set_plugin(name) ~= 1 then
         ...
         local v = w.config_get_plugin(name)
...
function config_cb(_, opt_name, opt_value)
   ...
   if g.defaults[name] then
      if g.defaults[name].type == "boolean" then
         opt_value = string_to_bool(opt_value)
      end
      if name == "show_mnemonics_table" then
         if not opt_value then
            g.mnemonic_keys = nil
         elseif not g.mnemonic_keys then
            sort_mnemonics_table()
         end
      end
...
function update_bar_items()
   w.bar_item_update(g.items.mode)
   w.bar_item_update(g.items.mnemonics)
end

function set_capture_mode(t)
   if t.active and not g.capture_param then
      local init_pos = w.buffer_get_integer(t.buffer, "input_pos") + 1
      g.capture_param = {
         length = 0,
         label = t.label,
         buffer = t.buffer,
      ...
      create_hooks()
      update_bar_items()
      if t.mode == "mnemonic" and g.config.show_mnemonics_table then
         show_bar(true, t.buffer)
      end
   end
end
...
function create_hooks()
   g.hooks = {
      w.hook_command_run("3000|/input return", "capture_ok_cb", ""),
      w.hook_command_run("3000|/input delete_previous_char", "capture_del_cb", "char"),
      w.hook_command_run("3000|/input delete_previous_word", "capture_del_cb", "word"),
      w.hook_command_run("2000|/input *", "capture_cancel_cb", "command"),
      w.hook_signal("3000|buffer_switch", "capture_cancel_cb", "buffer"),
      w.hook_signal("3000|window_switch", "capture_cancel_cb", "window"),
      w.hook_signal("3000|input_text_changed", "capture_input_cb", "")
   }
end

function remove_hooks()
   if g.hooks then
      for _, ptr in ipairs(g.hooks) do
         w.unhook(ptr)
      end
      g.hooks = nil
   end
...
function capture_ok_cb(_, buffer)
   ...
   if p.substring and p.substring ~= "" then
   ...
   if p.mode == "mnemonic" then
      show_bar(false, p.buffer)
   end
   g.capture_param = nil
   update_bar_items()
...
function capture_del_cb(mode, buffer)
   local p = g.capture_param
   if not p then
      return w.WEECHAT_RC_OK
   elseif not p.init_pos or
      not p.length or
      p.length < p.init_pos then
      capture_cancel_cb("internal", buffer)
      return w.WEECHAT_RC_OK_EAT
   end
   ...
   w.buffer_set(buffer, "input", left .. p.substring .. right)
   if p.mode == "mnemonic" then
      update_bar_items()
   end
   return w.WEECHAT_RC_OK_EAT
...
function capture_input_cb(_, _, buffer)
   ...
   if unicode.utf8.len(p.text) >= p.init_pos and p.length < p.max_length then
      ...
      if p.length >= p.max_length then
         capture_ok_cb(nil, buffer)
      elseif p.mode == "mnemonic" then
         update_bar_items()
      end
   end
   return w.WEECHAT_RC_OK
...
function capture_cancel_cb(from)
   remove_hooks()
   local buffer = g.capture_param.buffer
   if g.capture_param.mode == "mnemonic" then
      show_bar(false, buffer)
   end
   g.capture_param = nil
   update_bar_items()
   if from == "command" then
      return w.WEECHAT_RC_OK_EAT
   else
      return w.WEECHAT_RC_OK
   end
...
function cmd_input_mnemonic(buffer, args)
   ...
   if not args or args == "" then
      ...
      max_length = tonumber(args)
      if max_length < 2 then
         max_length = 2
      end
   end
   set_capture_mode({
      ...
      active = true,
      mode = "mnemonic",
      label = string.format("RFC-1345 Mnemonic (%d)", max_length),
      max_length = max_length,
      callback = input_mnemonic_callback
   })
end
...
function cmd_input_codepoint(buffer, args)
   set_capture_mode({
      ...
      active = true,
      mode = "codepoint",
      label = "UTF-8 Codepoint",
      max_length = 5,
      callback = input_codepoint_callback
   })
end
...
function command_cb(_, buffer, param)
   ...
   if action then
      local callback
      if action == "mnemonic" then
         callback = cmd_input_mnemonic
      elseif action == "codepoint" then
         callback = cmd_input_codepoint
      end
      if callback then
...
function item_mode_cb()
   if not g.capture_param then
      return ""
   else
      return w.color(g.config.mode_color) .. g.capture_param.label
   end
end

function generate_mnemonic_table(length, substring)
   local keys = g.mnemonic_keys[length]
   local mnemonics = g.mnemonics
   local text = ""
   for _, code in ipairs(keys) do
      if not substring or
         substring == "" or
         string.find(code, substring, 0, true) == 1 then
         text = string.format(
                  "%s%s%s %s%s  \n",
                  text,
                  w.color(g.config.code_color),
                  code,
                  w.color(g.config.char_color),
                  input_mnemonic_callback(code))
      end
   end
   if text == "" then
      return "No mnemonics found"
   else
      return text
   end
end

function item_mnemonics_cb()
   if not g.capture_param or not g.mnemonic_keys then
      return ""
   end

   local p = g.capture_param
   if not p.mode or p.mode ~= "mnemonic" then
      return ""
   elseif not g.mnemonic_keys[p.max_length] then
      return "No mnemonics found"
   end
   return generate_mnemonic_table(p.max_length, p.substring)
end

function sort_mnemonics_table()
   local mnemonics, t = g.mnemonics, {}
   for k, _ in pairs(mnemonics) do
      local length = #k
      if not t[length] then
         t[length] = {}
      end
      table.insert(t[length], k)
   end
   for length, keys in pairs(t) do
      table.sort(keys)
   end
   g.mnemonic_keys = t
end

function show_bar(flag, buffer)
...
function create_bar_and_items()
   g.items = {
      mode = g.script.name .. "_mode",
      mnemonics = g.script.name .. "_mnemonics"
   }

   w.bar_item_new(g.items.mode, "item_mode_cb", "")
   w.bar_item_new(g.items.mnemonics, "item_mnemonics_cb", "")

   local bar_ptr = w.bar_search(g.bar_name)
   if not bar_ptr or bar_ptr == "" then
      bar_ptr = w.bar_new(
         g.bar_name,                   -- name
         "on",                         -- hidden
         3000,                         -- priority
         ...
         "active",                     -- condition
         "bottom",                     -- position
         "columns_horizontal",         -- filling top/bottom
...
function setup()
   ...
   if g.config.show_mnemonics_table then
      sort_mnemonics_table()
   end
   create_bar_and_items()

   w.hook_command(
      g.script.name,
      "Input special UTF-8 character",
      "mnemonic [max-chars] || codepoint",
      "arg description",
      "mnemonic || codepoint",
      "command_cb",
      "")
...
-- extra ------------------------------------------------------------------------
      ...
      ["[ ]"] = 0x2610, -- BALLOT BOX
      ["[v]"] = 0x2611, -- BALLOT BOX WITH CHECK
      ["[x]"] = 0x2612, -- BALLOT BOX WITH X
